...
| Fan-out with `Send` | Several photos submitted together are described in one turn: the entry node returns one `Send` per photo, so each runs an isolated copy of the per-photo pipeline in the same superstep, their results accumulate through a reducer, and a join node composes one script in submission order for a single speech call. One photo is the same fan-out with one branch. | `clarif_eye.graph.photo_sends`, `clarif_eye.graph.build_photo_graph`, `clarif_eye.graph.compose_node`, `clarif_eye.state._accumulate_photo_results` |

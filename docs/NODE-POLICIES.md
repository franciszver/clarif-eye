...
- The `timeout` verdict would need revisiting if the pipeline moved from
  `graph.invoke()`/`graph.stream()` to their async counterparts across
  every node, which no part of this app does today.
- The `cache_policy` finding about caching failures is the default
  behavior; a custom cache key function or a wrapper that inspects the
  outcome before writing to the cache could close that gap, but at that
  point it is doing the same work our existing cache already does.
